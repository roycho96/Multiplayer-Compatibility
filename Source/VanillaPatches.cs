...
namespace Multiplayer.Compat
{
    ...
    public static class VanillaPatches
    {
        public static void Apply()
        {
            ...
            TryPatch("PatchMathfTranscendentals",     PatchMathfTranscendentals);
            TryPatch("PatchFloatAccumulatorSetters",  PatchFloatAccumulatorSetters);
            TryPatch("PatchFloatAccumulatorPostTick", PatchFloatAccumulatorPostTick);
            ...
            TryPatch("PatchExtraFloatAccumulators",   PatchExtraFloatAccumulators);
            TryPatch("PatchScribeValuesLookFloat",    PatchScribeValuesLookFloat);
        ...
        /// <summary>
        /// STEP 16 — runtime auto-quantize of all <c>float</c> values that
        /// pass through <c>Scribe_Values.Look&lt;float&gt;</c>. Desync-26
        /// confirmed user's hypothesis: long MP sessions accumulate float
        /// drift in many fields STEP 9's setter patches don't cover (pawn
        /// records, thought intensities, psyfocus, etc.). A one-time manual
        /// cleanup (quantize_mpreplay.py) resets load-time values but
        /// per-tick drift reintroduces divergence within minutes.
        ///
        /// Fix: quantize every float at every save/load via the Scribe
        /// pipeline. Autosaves (typically once per in-game day) now act as
        /// automatic drift reset points. Any accumulated sub-ULP drift is
        /// flushed at each save boundary, bounding total drift to a single
        /// autosave interval. Catches 100% of serialized floats regardless
        /// of field name — no per-field reflection needed.
        ///
        /// <para>Prefix fires for <c>Saving</c> mode (quantize before the
        /// XML is written), postfix for <c>LoadingVars</c> mode (quantize
        /// after the XML value is deserialized into the ref parameter).</para>
        /// </summary>
        private static void PatchScribeValuesLookFloat()
        {
            var scribeType = AccessTools.TypeByName("Verse.Scribe_Values");
            if (scribeType == null)
            {
                Log.Warning("MPCompat :: Verse.Scribe_Values not found — Scribe float quantize skipped");
                return;
            }

            // Scribe_Values.Look<T>(ref T value, string label, T defaultValue, bool forceSave)
            MethodInfo genericLook = null;
            foreach (var m in scribeType.GetMethods(BindingFlags.Public | BindingFlags.Static))
            {
                if (m.Name != "Look") continue;
            ...
            if (genericLook == null)
            {
                Log.Warning("MPCompat :: Scribe_Values.Look<T> generic definition not found");
                return;
            }

            MethodInfo floatLook;
            try { floatLook = genericLook.MakeGenericMethod(typeof(float)); }
            catch (Exception e)
            {
                Log.Warning($"MPCompat :: MakeGenericMethod(float) failed: {e.Message}");
                return;
            }

            MpCompat.harmony.Patch(floatLook,
                prefix:  new HarmonyMethod(AccessTools.Method(typeof(VanillaPatches), nameof(ScribeLookFloatPrefix))),
                postfix: new HarmonyMethod(AccessTools.Method(typeof(VanillaPatches), nameof(ScribeLookFloatPostfix))));
            Log.Message("MPCompat :: Scribe_Values.Look<float> — auto-quantize prefix (save) + postfix (load) installed (STEP 16)");
        }

        public static void ScribeLookFloatPrefix(ref float value)
        {
            // Only active during serialization writes: quantize BEFORE the
            // body writes the XML text.
            if (Scribe.mode == LoadSaveMode.Saving)
                value = Quantize(value);
        }

        public static void ScribeLookFloatPostfix(ref float value)
        {
            // Only active during deserialization reads: quantize AFTER the
            // body has parsed the XML text into value.
            if (Scribe.mode == LoadSaveMode.LoadingVars)
                value = Quantize(value);
        }

        /// <summary>
        /// STEP 14e — quantize additional float accumulators that tick
        /// continuously and can drift 1 ULP over many ticks. Desync-23 and
        /// Desync-24 both showed <c>Trace count: 0</c> — the map state
        /// diverged without any Rand being consumed in the trace window,
        /// implying pure float drift. STEP 9 quantized
        /// <c>Hediff.Severity</c> and <c>Need.CurLevel</c> setters but
        /// missed other tick-accumulated floats. This step extends
        /// coverage to the most prolific remaining candidates:
        /// <list type="bullet">
        /// <item><c>CompPowerBattery.storedEnergy</c> — tick-updated by
        ///   PowerNet distribution; complex float math (power in/out,
        ///   losses). Patched via postfix on <c>ChangeStoredEnergy</c> that
        ///   re-quantizes the backing field.</item>
        /// <item><c>CompRottable.RotProgress</c> setter — rot tick
        ///   accumulator.</item>
        /// </list>
        /// Both targets are ThingComps (not MapComponents), expected safe
        /// per the STEP 14d precedent.
        /// </summary>
        private static FieldInfo compPowerBattery_storedEnergyField;

        private static void PatchExtraFloatAccumulators()
        {
            // CompPowerBattery.storedEnergy — CompPowerBattery does NOT
            // override CompTickInterval (it's inherited from base ThingComp),
            // so Harmony refuses to patch the subclass reference (Desync-27
            // log: "Patch the declared method virtual
            // Verse.ThingComp::CompTickInterval(Int32 delta) instead").
            // Workaround: patch the BASE ThingComp.CompTickInterval with a
            // postfix that filters by type. The postfix fires for every
            // ThingComp tick but only does work when __instance is
            // CompPowerBattery. Overhead per call is a single type check.
            var batteryType = AccessTools.TypeByName("RimWorld.CompPowerBattery");
            if (batteryType != null)
            {
                compPowerBattery_storedEnergyField = AccessTools.Field(batteryType, "storedEnergy");
                if (compPowerBattery_storedEnergyField != null)
                {
                    var baseType = AccessTools.TypeByName("Verse.ThingComp");
                    var tick = baseType != null
                        ? AccessTools.Method(baseType, "CompTickInterval", new[] { typeof(int) })
                        : null;
                    if (tick != null)
                    {
                        MpCompat.harmony.Patch(tick,
                            postfix: new HarmonyMethod(AccessTools.Method(typeof(VanillaPatches), nameof(QuantizeBatteryStoredEnergyPostfix))));
                        Log.Message("MPCompat :: ThingComp.CompTickInterval — storedEnergy postfix installed with CompPowerBattery type filter");
                    }
                    else
                    {
                        Log.Warning("MPCompat :: Verse.ThingComp.CompTickInterval not found — battery quantize skipped");
                    }
                }
                else
                {
                    Log.Warning("MPCompat :: CompPowerBattery.storedEnergy field not found — battery quantize skipped");
                }
            }

            // CompRottable.RotProgress setter prefix quantize
            var rotType = AccessTools.TypeByName("RimWorld.CompRottable");
            if (rotType != null)
            {
                var rotSetter = AccessTools.PropertySetter(rotType, "RotProgress");
                if (rotSetter != null)
                {
                    MpCompat.harmony.Patch(rotSetter,
                        prefix: new HarmonyMethod(AccessTools.Method(typeof(VanillaPatches), nameof(QuantizeValuePrefix))));
                    Log.Message("MPCompat :: CompRottable.RotProgress setter — quantize prefix installed (Desync-23/24 fix)");
                }
                else
                {
                    Log.Warning("MPCompat :: CompRottable.RotProgress setter not found, skipped");
                }
            }
        }

        public static void QuantizeBatteryStoredEnergyPostfix(ThingComp __instance)
        {
            // Runs for every ThingComp.CompTickInterval call. Filter by type:
            // only CompPowerBattery instances carry a storedEnergy float that
            // can drift.
            if (__instance == null || compPowerBattery_storedEnergyField == null) return;
            if (__instance.GetType() != compPowerBattery_storedEnergyField.DeclaringType) return;
            try
            {
                var cur = (float)compPowerBattery_storedEnergyField.GetValue(__instance);
                compPowerBattery_storedEnergyField.SetValue(__instance, Quantize(cur));
            }
            catch { /* best-effort */ }
        }
        ...
        /// <summary>
        /// Zero the bottom 4 mantissa bits of a float. This strips ~2^-19
        /// relative precision, which is 16x the typical 1–2 ULP divergence
        /// observed between libm implementations (glibc / musl / ucrt).
        /// Game logic in RimWorld never cares about sub-1e-6 float precision.
        /// Integer-only, so the quantization itself introduces zero secondary
        /// drift.
        /// </summary>
        public static unsafe float Quantize(float f)
        {
            if (float.IsNaN(f) || float.IsInfinity(f)) return f;
            int bits = *(int*)&f;
            bits = (int)((uint)bits & 0xFFFFFFF0u);
            return *(float*)&bits;
        }

        public static void QuantizeResultPostfix(ref float __result)
        {
            __result = Quantize(__result);
        }

        public static void QuantizeValuePrefix(ref float value)
        {
            value = Quantize(value);
        }

        /// <summary>
        /// Postfix-quantize the results of every transcendental function in
        /// UnityEngine.Mathf. These are the only FP operations in the vanilla
        /// math API that can differ bit-for-bit across x64 CPUs / OSes — basic
        /// +-*/ and sqrt go through SSE2 and are IEEE-754 deterministic.
        /// </summary>
        private static void PatchMathfTranscendentals()
        {
            var postfix = new HarmonyMethod(AccessTools.Method(typeof(VanillaPatches), nameof(QuantizeResultPostfix)));

            // Transcendentals only. Sqrt is deterministic (SSE2 sqrtss), skip it.
            var targetNames = new System.Collections.Generic.HashSet<string>
            {
                "Pow", "Exp", "Log", "Log10",
                "Sin", "Cos", "Tan",
            ...
            foreach (var m in typeof(Mathf).GetMethods(BindingFlags.Public | BindingFlags.Static))
            {
                if (!targetNames.Contains(m.Name)) continue;
                if (m.ReturnType != typeof(float)) continue;
                try
                {
                    MpCompat.harmony.Patch(m, postfix: postfix);
                    patched++;
                }
                catch (Exception e)
                {
                    skipped++;
                    Log.Warning($"MPCompat :: Failed to patch Mathf.{m.Name}: {e.Message}");
                }
            }

            Log.Message($"MPCompat :: Quantize-patched {patched} Mathf transcendentals ({skipped} failed)");
        }

        /// <summary>
        /// Prefix-quantize the <c>value</c> going into the two most heavily
        /// accumulated float fields in the engine: <c>Hediff.Severity</c> and
        /// <c>Need.CurLevel</c>. Both are written every pawn tick and carry
        /// state across ticks, so a sub-ULP drift on one side compounds into a
        /// branch-dividing difference within seconds.
        ///
        /// Patching the property setter (rather than every stfld site) catches
        /// 100% of external mutations. Direct <c>severityInt</c>/<c>curLevelInt</c>
        /// writes from inside Verse still bypass this — add a targeted stfld
        /// transpiler if a future desync trace fingers those paths.
        /// </summary>
        private static void PatchFloatAccumulatorSetters()
        {
            var prefix = new HarmonyMethod(AccessTools.Method(typeof(VanillaPatches), nameof(QuantizeValuePrefix)));

            var severitySetter = AccessTools.PropertySetter(typeof(Hediff), nameof(Hediff.Severity));
            if (severitySetter != null)
            {
                TryPatch("Hediff.Severity setter quantize", () =>
                {
                    MpCompat.harmony.Patch(severitySetter, prefix: prefix);
                    Log.Message("MPCompat :: Quantize-patched Hediff.Severity setter");
                });
            }
            else
            {
                Log.Warning("MPCompat :: Hediff.Severity setter not found — patch skipped");
            }

            var curLevelSetter = AccessTools.PropertySetter(typeof(Need), nameof(Need.CurLevel));
            if (curLevelSetter != null)
            {
                TryPatch("Need.CurLevel setter quantize", () =>
                {
                    MpCompat.harmony.Patch(curLevelSetter, prefix: prefix);
                    Log.Message("MPCompat :: Quantize-patched Need.CurLevel setter");
                });
            }
            else
            {
                Log.Warning("MPCompat :: Need.CurLevel setter not found — patch skipped");
            }
        }

        /// <summary>
        /// Latent risk #3 mitigation: insurance against direct <c>stfld</c> to
        /// <c>severityInt</c> / <c>curLevelInt</c> from inside Verse that
        /// bypass the setter patches above. We add a postfix on the per-hediff
        /// and per-need tick entry points that re-quantizes the backing field
        /// after all intra-tick mutations complete. This guarantees the value
        /// stored across tick boundaries is always quantized, regardless of
        /// which mutation path was used inside the tick.
        ///
        /// This is a backstop — <see cref="PatchFloatAccumulatorSetters"/> is
        /// still the primary defense for writes through the property. The
        /// postfix here only matters if Verse internals stfld-bypass the
        /// setter, which is rare but possible.
        /// </summary>
        private static void PatchFloatAccumulatorPostTick()
        {
            // Hediff.TickInterval(int) — called by HediffSet for every hediff
            // on a pawn each tick. Postfix re-quantizes severityInt after the
            // tick body runs.
            var hediffTick = AccessTools.Method(typeof(Hediff), "TickInterval", new[] { typeof(int) })
                            ?? AccessTools.Method(typeof(Hediff), "Tick");
            if (hediffTick != null)
            {
                TryPatch("Hediff tick severityInt requantize", () =>
                {
                    MpCompat.harmony.Patch(hediffTick,
                        postfix: new HarmonyMethod(AccessTools.Method(typeof(VanillaPatches), nameof(RequantizeHediffSeverityPostfix))));
                    Log.Message($"MPCompat :: Quantize-postfix on Hediff.{hediffTick.Name} — covers direct severityInt writes");
                });
            }
            else
            {
                Log.Warning("MPCompat :: Hediff tick method not found — severityInt backstop skipped");
            }

            // Need.NeedInterval() is abstract on the base class (no body →
            // Harmony BadImageFormatException). Each concrete Need subclass
            // provides its own body. Enumerate subclasses and patch every
            // declared override that actually has a body.
            var needPostfix = new HarmonyMethod(AccessTools.Method(typeof(VanillaPatches), nameof(RequantizeNeedCurLevelPostfix)));
            var needBase = typeof(Need);
            Type[] needTypes;
            try { needTypes = needBase.Assembly.GetTypes(); }
            catch (ReflectionTypeLoadException rtle) { needTypes = rtle.Types.Where(t => t != null).ToArray(); }

            int needPatched = 0;
            foreach (var t in needTypes)
            {
                if (t == null || t.IsAbstract || !needBase.IsAssignableFrom(t)) continue;
                var m = AccessTools.DeclaredMethod(t, "NeedInterval");
                if (m == null || m.IsAbstract) continue;
                var captured = m;
                var tName = t.Name;
                if (TryPatch($"{tName}.NeedInterval curLevelInt requantize", () =>
                {
                    MpCompat.harmony.Patch(captured, postfix: needPostfix);
                }))
                    needPatched++;
            }
            if (needPatched > 0)
                Log.Message($"MPCompat :: Quantize-postfix on {needPatched} Need.NeedInterval override(s) — covers direct curLevelInt writes");
            else
                Log.Warning("MPCompat :: No Need.NeedInterval overrides found — curLevelInt backstop skipped");
        }

        public static void RequantizeHediffSeverityPostfix(Hediff __instance)
        {
            if (__instance != null)
                __instance.severityInt = Quantize(__instance.severityInt);
        }

        public static void RequantizeNeedCurLevelPostfix(Need __instance)
        {
            if (__instance != null)
                __instance.curLevelInt = Quantize(__instance.curLevelInt);
        }
